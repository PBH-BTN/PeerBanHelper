<template>
  <a-space direction="vertical" fill>
    <a-form-item
      ...
      :tooltip="t('page.settings.tab.profile.module.ruleSubscribe.enable.tips')"
      field="model.enabled"
    >
    ...
    <a-typography-text>
      <i18n-t keypath="page.settings.tab.profile.module.ruleSubscribe.subscribe">
        <template #link>
          <a-link @click="goto('rule_management_subscribe')">{{
            t('page.settings.tab.profile.module.ruleSubscribe.subscribe.link')
          }}</a-link>
        </template>
      </i18n-t>
    </a-typography-text>
...
import type { IpAddressBlockerRules } from '@/api/model/profile'
