...
@Component
public class ClientNameRunner extends AbstractRunner {
    private final RunnerRegistryManager runnerRegistryManager;

    public ClientNameRunner(RunnerRegistryManager runnerRegistryManager) {
        super();
    ...
    public @NotNull RunnerAction shouldBanPeer(@NotNull Torrent torrent, @NotNull Peer peer, @NotNull Downloader downloader) {
        ...
        RuleMatchResult matchResult = RuleParser.matchRule(ruleList, peer.getClientName());
        if (matchResult.hit()) {
