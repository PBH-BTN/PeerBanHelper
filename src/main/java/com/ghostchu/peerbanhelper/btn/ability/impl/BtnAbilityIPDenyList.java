...
@Slf4j
public final class BtnAbilityIPDenyList extends AbstractBtnAbility {
    ...
    private void loadCacheFile() {
        ...
        if (cacheValue != null) {
            ...
            ipMatcher.setData("BTN DenyList (Local Cache)", List.of(associativeTries));
            ruleVersion = cacheVersion;
            log.debug("[BTN DenyList] Loaded {} IP rules from cache file. Cached version: {}", loaded, cacheVersion);
        }
    }
    ...
    public TranslationComponent getDescription() {
        return new TranslationComponent(Lang.BTN_ABILITY_IP_DENYLIST_DESCRIPTION,
                ruleVersion,
                ipMatcher.size()
        );
    }
    ...
    public void load() {
        try {
            loadCacheFile();
            setLastStatus(true, new TranslationComponent(Lang.BTN_ABILITY_IP_DENYLIST_LOADED_FROM_CACHE, ruleVersion, ipMatcher.size()));
        } catch (Exception e) {
            log.error(tlUI(Lang.BTN_ABILITY_IP_DENYLIST_LOAD_FAILED_FROM_CACHE), e);
    ...
    private void updateRule() {
        btnNetwork.getBackgroundTaskManager().addTaskAsync(new FunctionalBackgroundTask(new TranslationComponent(Lang.BTN_ABILITY_DENY_LIST_SYNC_SERVER), (task, callback) -> {
