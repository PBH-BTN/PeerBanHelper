...
COPY . /build
WORKDIR /build
COPY --from=build_web webui/dist peerbanhelper-impl/src/main/resources/static
RUN apk add git  && \
...
WORKDIR /app
VOLUME /tmp
COPY --from=build build/peerbanhelper-impl/target/libraries /app/libraries
COPY --from=build build/peerbanhelper-impl/target/PeerBanHelper.jar /app/PeerBanHelper.jar
