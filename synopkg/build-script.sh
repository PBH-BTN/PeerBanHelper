...
sed -i "s/@IMAGE@/ghostchu\/peerbanhelper-snapshot:sha-${github.sha}/g" /toolkit/source/PeerBanHelperPackage/target/app/docker-compose.yaml
cat /toolkit/source/PeerBanHelperPackage/target/app/docker-compose.yaml
sed -i "s/@PBH_VERSION@/${steps.get-version.outputs.version}/g" /toolkit/source/PeerBanHelperPackage/INFO.sh
